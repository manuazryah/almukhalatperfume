    ...
    public function actionGetadress() {
        if (yii::$app->request->isAjax) {
            ...
            if (isset($id)) {
                ...
                if ($address) {
                    echo json_encode(['rslt' => 'success', 'name' => $address->name, 'address' => $address->address, 'landmark' => $address->landmark, 'location' => $address->location, 'emirate' => $address->emirate, 'post_code' => $address->post_code, 'mobile_number' => $address->mobile_number, 'country_code' => $address->country_code, 'street' => $address->street_number]);
                } else {
...
//    }
    ...
    public function actionMail() {
        ...
        $to = 'Sales@perfumedunia.com,sabitha@azryah.com';
        $message = 'hii';
        $headers = 'MIME-Version: 1.0' . "\r\n";
        $headers .= "Content-type: text/html; charset=iso-8859-1" . "\r\n" .
                "From: noreply@perfumedunia.com";
        mail($to, $subject, $message, $headers);
    }

    public function actionTest() {
        $message = Yii::$app->mailer->compose('test') // a view rendering result becomes the message body here
                ->setFrom('noreply@perfumedunia.com')
                ->setTo('sabitha@azryah.com,sales@perfumedunia.com')
                ->setSubject('test');
        $message->send();
    }
