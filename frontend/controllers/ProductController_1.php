...
class ProductController extends \yii\web\Controller {
    ...
    public function CategorizedData($dataProvider, $category) {
        ...
        if ($category == "body-mist") {
            $product_type = 3;
            return $dataProvider->query->andWhere(['product_type' => $product_type]);
        } elseif ($category == "scent-diffuser") {
            $main_category = 3;
            return $dataProvider->query->andWhere(['main_category' => $main_category]);
        } elseif ($category == "gift") {
            $main_category = 2;
            return $dataProvider->query->andWhere(['main_category' => $main_category]);
        } elseif ($category == "special-offer") {
            $main_category = 4;
            return $dataProvider->query->andWhere(['main_category' => $main_category]);
        } elseif ($category == "women") {
            $category_model = Category::find()->where(['status' => 1, 'category_code' => $category])->one();
            if ($category_model) {
                return $dataProvider->query->andWhere(['category' => $category_model->id]);
            } else {
