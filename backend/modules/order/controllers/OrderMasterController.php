...
//        $dataProvider->query->andWhere(['status' => '4'])->orWhere(['status' => '5']);
    ...
    public function actionView($id) {
        ...
        $order_promotions = OrderPromotions::find()->where(['order_master_id' => $order_master->id])->sum('promotion_discount');
    ...
    public function actionFailedOrder($order_status = NULL) {
        $searchModel = new OrderMasterSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->andWhere(['payment_status' => 2]);
        $dataProvider->query->andWhere(['<>', 'status', 5]);

        return $this->render('failedindex', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
//                    'order_status' => $order_status,
        ]);
...
//                    }
    ...
    /**
     * This function change return approve
     * @return mixed
     */
...
//                     exit;
    ...
    public function actionOrderReport() {
        ...
        if (Yii::$app->request->post()) {
            ...
            if (isset($_POST['OrderMasterSearch']['admin_status']) && $_POST['OrderMasterSearch']['admin_status'] != '') {
                $query->andWhere(['admin_status' => $_POST['OrderMasterSearch']['admin_status']]);
                $admin_status = $_POST['OrderMasterSearch']['admin_status'];
            }
